...
export function DonelanJONSWAP(w){
    ...
    if (Omega >= 1){
        var gammat = 1.7 + 6 * Math.log(Omega);
    }
    else{
        var gammat = 1.7
    }
...
export function OchiSingle(w, l, wm, hs){
    ...
    var s = 0.25 * Math.pow(aux * Math.pow(wm, 4), l)/gamma_sp(l) * Math.pow(hs,2)/Math.pow(w, 4*l + 1) * Math.exp(-aux * Math.pow(wm/w, 4))
    return s
}



export function Ochi(w){
    var s = OchiSingle(w, l1, wm1,hs1) + OchiSingle(w, l2, wm2, hs2)
    return s
}
    


export function OchiHs(w){
...
const SPECTRUM_TESSENDORF = 0
const SPECTRUM_PIERSON_MOSKOWITZ = 1
const SPECTRUM_JONSWAP = 2
const SPECTRUM_TMA = 3
const SPECTRUM_DONELAN = 4
const SPECTRUM_PIERSON_MOSKOWITZ_HS = 5
const SPECTRUM_BRETSCHNEIDER_HS = 6
const SPECTRUM_UNIFIED = 7
const SPECTRUM_OCHI = 8
const SPECTRUM_OCHI_HS = 9

  
export function getPeakW(spectrum){

    
    if (spectrum == SPECTRUM_PIERSON_MOSKOWITZ){
        return PiersonMoskowitzPeakW()
    }
    
    else if (spectrum == SPECTRUM_JONSWAP){
        return 22 * (g / windSpeed) * Math.pow(windSpeed*windSpeed / (g * fetchValue), 0.33)
    }
    
    else if (spectrum == SPECTRUM_TMA){
        return 22 * (g / windSpeed) * Math.pow(windSpeed*windSpeed / (g * fetchValue), 0.33)
    }
    
    else if (spectrum == SPECTRUM_DONELAN){
        dimensionlessFetch = g * fetchValue / Math.pow(windSpeed, 2)    
        Omega = 11.6 * Math.pow(dimensionlessFetch,-0.23)
        return Omega * g / windSpeed
    }
    
    else if (spectrum == SPECTRUM_PIERSON_MOSKOWITZ_HS){
        return PiersonMoskowitzHsPeakW()
    }
    else{
        return 1.0;
    }
